![Tổng quan bài toán churn](<Screenshot 2026-03-22 224527.png>)
...
> Lưu ý: trong các đoạn code EDA, nên dùng thống nhất biến `df` để tránh lỗi khi chạy.

Thông tin nhanh về dataset Telco:
- Tổng số dòng: **7043**
- Tổng số cột: **21**
- Biến mục tiêu: **`Churn`**
...
#Bổ sung 2 biến theo yêu cầu phân tích
fig, axes = plt.subplots(1, 2, figsize=(12, 4))
...
3. Kiểm tra lỗi
```python
#Kiểm tra số lượng thiếu trên từng cột
...
- LabelEncoder sẽ tự ánh xạ theo thứ tự nhãn (thường là `Female = 0`, `Male = 1`)
- Không nên hard-code thủ công 0/1 khi dùng `LabelEncoder`
...
Lưu ý quan trọng:
- Nên chọn **một** cách preprocessing chính khi train model (pipeline hoặc xử lý tay), tránh trộn cả hai cách để kết quả khó đối chiếu.
- Nếu đã dùng `ColumnTransformer + OneHotEncoder` trong pipeline, phần `LabelEncoder + get_dummies` chỉ nên xem là phần minh họa tham khảo.
...
```
### 4.1.1. Bổ sung theo phân công: bộ 3 mô hình chính (Logistic, Random Forest, XGBoost)
Để bám đúng yêu cầu phân công, có thể dùng bộ mô hình sau:
...
Yếu tố ảnh hưởng mạnh đến *Churn* là thời gian sử dụng dịch vụ (*tenure*), loại hợp đồng (*contract type*), dịch vụ internet và phí hàng tháng (*monthly charges*).

Dataset mẫu *Telco Customer Churn* trên Kaggle chưa phản ánh đầy đủ thực tế doanh nghiệp; ngoài các yếu tố trong dữ liệu, churn còn chịu tác động bởi cạnh tranh thị trường, xu hướng tiêu dùng và biến động kinh tế.

Kết luận vận hành:
- Theo bảng so sánh trong notebook, mô hình tốt nhất được chọn theo tiêu chí `ROC AUC`.
- Mô hình này có thể dùng để xếp hạng khách hàng rủi ro cao và hỗ trợ ưu tiên chiến dịch giữ chân.
